...
public final class Utils {
    ...
    public static Interval getConflictInterval(MEDEdge e, MEDEdge c, double startTimeC, double speed,
            double fullLengthTime, double crossingDelay, MEDAnimation.MorphType morphType) {
    ...
    /**
     * Computes the y-value of the cubic Bezier curve from (0,0) to (1,1) via
     * control points (x1,y1) and (x2,y2) at x-coordinate x
     */
    public static double cubicBezierCurveAt(double x1, double y1, double x2, double y2, double x) {
        ...
        if (x1 == y1 && x2 == y2)
            return x;
        if (Math.abs(1.0 - x) < epsilon)
            return 1.0;
        if (Math.abs(0.0 - x) < epsilon)
            return 0.0;

        double t = approximateT(ax, bx, cx, ay, by, cy, x, epsilon);
        return ((ay * t + by) * t + cy) * t;
    }

    private static double approximateT(double ax, double bx, double cx, double ay, double by, double cy, double x,
            double epsilon) {
